...
func (hq *HabitQuery) Order(o ...habit.OrderOption) *HabitQuery {
...
func (hq *HabitQuery) All(ctx context.Context) ([]*Habit, error) {
	...
	return withInterceptors[[]*Habit](ctx, hq, qr, hq.inters)
}
...
func (hq *HabitQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (hq *HabitQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, hq, querierCount[*HabitQuery](), hq.inters)
}
...
func (hq *HabitQuery) Clone() *HabitQuery {
	...
	return &HabitQuery{
		config:     hq.config,
		ctx:        hq.ctx.Clone(),
		order:      append([]habit.OrderOption{}, hq.order...),
...
func (hq *HabitQuery) Aggregate(fns ...AggregateFunc) *HabitSelect {
...
func (hq *HabitQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Habit, error) {
	var (
		nodes = []*Habit{}
		_spec = hq.querySpec()
	)
...
func (hq *HabitQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(habit.Table, habit.Columns, sqlgraph.NewFieldSpec(habit.FieldID, field.TypeInt))
	_spec.From = hq.sql
...
func (hgb *HabitGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, hgb.build.ctx, ent.OpQueryGroupBy)
	if err := hgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*HabitQuery, *HabitGroupBy](ctx, hgb.build, hgb, hgb.build.inters, v)
}

func (hgb *HabitGroupBy) sqlScan(ctx context.Context, root *HabitQuery, v any) error {
...
func (hs *HabitSelect) Aggregate(fns ...AggregateFunc) *HabitSelect {
	hs.fns = append(hs.fns, fns...)
	return hs
}

// Scan applies the selector query and scans the result into the given value.
func (hs *HabitSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, hs.ctx, ent.OpQuerySelect)
	if err := hs.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*HabitQuery, *HabitSelect](ctx, hs.HabitQuery, hs, hs.inters, v)
}

func (hs *HabitSelect) sqlScan(ctx context.Context, root *HabitQuery, v any) error {
